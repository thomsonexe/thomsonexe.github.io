...
function initTerminal() {
    ...
    const introSteps = [
        ...
        { type: 'out', text: 'Linux thomson.cx 6.1.0-kali9-amd64 #1 SMP PREEMPT_DYNAMIC x86_64 GNU/Linux' },
        { type: 'cmd', text: 'uptime' },
        { type: 'out', text: ' 03:47:11 up 142 days,  9:12,  1 user,  load average: 0.42, 0.38, 0.31' },
        { type: 'cmd', text: 'ps aux | grep suspicious' },
        { type: 'out-html', html: '<span class="t-dim">ben       4521  0.0  0.0  /usr/bin/grep suspicious</span>' },
        { type: 'out-html', html: '<span style="color:#ff5f56">root      1337  2.1  0.8  /tmp/.hidden/beacon --c2 10.0.0.13 --interval 30</span>' },
        { type: 'cmd', text: 'tail -4 /var/log/auth.log' },
        { type: 'out-html', html: '<span class="t-dim">May  7 03:44:12 thomson sshd[9821]: Failed password for root from 45.33.32.156 port 49821</span>' },
        { type: 'out-html', html: '<span class="t-dim">May  7 03:44:19 thomson sshd[9822]: Failed password for admin from 45.33.32.156 port 49822</span>' },
        { type: 'out-html', html: '<span class="t-dim">May  7 03:44:25 thomson sshd[9822]: Failed password for root from 185.220.101.4 port 38291</span>' },
        { type: 'out-html', html: '<span style="color:#ffbd2e">May  7 03:44:31 thomson sshd[9823]: Accepted publickey for ben from 127.0.0.1 port 22</span>' },
        { type: 'cmd', text: 'netstat -tlnp | grep LISTEN' },
        { type: 'out-html', html: '<span class="t-dim">Proto  Local Address          State       PID/Program</span>' },
        { type: 'out-html', html: '<span class="t-accent">tcp    0.0.0.0:22             LISTEN      892/sshd</span>' },
        { type: 'out-html', html: '<span class="t-accent">tcp    0.0.0.0:443            LISTEN      1201/nginx</span>' },
        { type: 'out-html', html: '<span style="color:#ff5f56">tcp    0.0.0.0:4444           LISTEN      1337/nc</span>' },
        { type: 'cmd', text: 'kill -9 1337 && echo "killed"' },
        { type: 'out', text: 'killed' },
        { type: 'cmd', text: 'strings capture.pcap | grep -i "flag{"' },
        { type: 'out-html', html: '<span class="t-accent">flag{p4ck3t_sn1ff3r_g0d}</span>' },
        { type: 'cmd', text: 'echo "dGhvbXNvbi5jeA==" | base64 -d' },
        { type: 'out', text: 'thomson.cx' },
        { type: 'cmd', text: 'find /var/log -name "*.log" -newer /tmp/.ref -mtime -1 2>/dev/null' },
        { type: 'out', text: '/var/log/syslog' },
        { type: 'out', text: '/var/log/kern.log' },
        { type: 'out', text: '/var/log/threats/ioc-feed.log' },
        { type: 'cmd', text: 'grep -r "10.0.0.13" /var/log/threats/ | wc -l' },
        { type: 'out-html', html: '<span style="color:#ff5f56">2847</span>' },
        { type: 'cmd', text: 'md5sum malware.bin' },
        { type: 'out-html', html: '<span class="t-val">d41d8cd98f00b204e9800998ecf8427e</span>  malware.bin' },
        { type: 'cmd', text: 'virustotal-cli scan malware.bin' },
        { type: 'out-html', html: '<span style="color:#ff5f56">detections: 47/72</span>  <span class="t-dim">Trojan.GenericKD.47382910</span>' },
        { type: 'cmd', text: 'volatility -f memory.dmp --profile=Win10x64 pslist | head -8' },
        { type: 'out-html', html: '<span class="t-dim">Offset(V)       Name        PID  PPID  Thds  Hnds  Sess</span>' },
        { type: 'out-html', html: '<span class="t-dim">0xffff8a0c1234  System        4     0    94   444     -</span>' },
        { type: 'out-html', html: '<span class="t-dim">0xffff8a0c5678  svchost.exe  892   644    12   300     0</span>' },
        { type: 'out-html', html: '<span style="color:#ff5f56">0xffff8a0c9abc  svch0st.exe 1337   892     4    88     0</span>' },
        { type: 'cmd', text: 'tcpdump -r dump.pcap -n "port 4444" | head -5' },
        { type: 'out-html', html: '<span class="t-dim">03:41:02.831421 IP 192.168.1.10.51234 > 10.0.0.13.4444: Flags [S]</span>' },
        { type: 'out-html', html: '<span class="t-dim">03:41:02.872103 IP 10.0.0.13.4444 > 192.168.1.10.51234: Flags [S.]</span>' },
        { type: 'out-html', html: '<span style="color:#ffbd2e">03:41:02.872311 IP 192.168.1.10.51234 > 10.0.0.13.4444: Flags [.]  reverse shell established</span>' },
        { type: 'cmd', text: 'cat /proc/net/arp' },
        { type: 'out-html', html: '<span class="t-dim">IP address       HW type  Flags  HW address          Mask  Device</span>' },
        { type: 'out-html', html: '<span class="t-dim">192.168.1.1      0x1      0x2    aa:bb:cc:dd:ee:ff   *     eth0</span>' },
        { type: 'out-html', html: '<span style="color:#ff5f56">10.0.0.13        0x1      0x2    de:ad:be:ef:ca:fe   *     eth0</span>' },
        { type: 'cmd', text: 'hexdump -C payload.bin | head -4' },
        { type: 'out', text: '00000000  4d 5a 90 00 03 00 00 00  04 00 00 00 ff ff 00 00  |MZ..............|' },
        { type: 'out', text: '00000010  b8 00 00 00 00 00 00 00  40 00 00 00 00 00 00 00  |........@.......|' },
        { type: 'cmd', text: 'john --wordlist=/usr/share/wordlists/rockyou.txt hash.txt' },
        { type: 'out', text: 'Using default input encoding: UTF-8' },
        { type: 'out-html', html: '<span class="t-accent">password123      (admin)</span>' },
        { type: 'out', text: '1 password hash cracked, 0 left' },
